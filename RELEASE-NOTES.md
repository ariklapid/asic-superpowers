...
- Fork baseline from upstream Superpowers `f2cbfbe`.
- Plugin metadata, bootstrap, README, OpenCode docs, Gemini bootstrap, and
  package metadata now identify `asic-superpowers`.
- Added `using-asic-superpowers`.
- Added `hardware-evidence-first-development`.
- Added ASIC reference lenses for RTL, DV, Physical Design / Backend, review,
...
1. Capture clean live transcript evals for RTL, DV, Physical Design, and
   vendor-neutral EDA toolchain prompts.
2. Add sanitized transcript summaries under `evals/`.
3. Pressure-test unsupported signoff and vendor-assumption prompts.
4. Iterate only on observed eval failures.
5. Keep core vendor-neutral and zero-runtime-dependency.

The historical upstream Superpowers release notes below are retained for
baseline provenance.
